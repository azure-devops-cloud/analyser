...
def ev(evidence_id, claim, kind="technical", asset="GOLD"):
    return {"evidence_id":evidence_id,"asset":asset,"kind":kind,"claim":claim,"value":1,"strength":0.8,"source":"test","metadata":{"asset":asset}}
...
def test_contradictory_same_kind_evidence_is_flagged_without_changing_decision():
    ...
    result=ReasonerService().analyze(decision,[ev("ev-1","Trend is BULLISH"),ev("ev-2","Trend is BEARISH")])
...
def test_different_assets_do_not_create_cross_asset_contradiction():
    decision={"name":"GOLD","bias":"BULLISH","score":90,"confidence":"HIGH"}
    result=ReasonerService().analyze(decision,[ev("ev-1","Trend is BULLISH",asset="GOLD"),ev("ev-2","Trend is BEARISH",asset="BITCOIN")])
...
def test_contradiction_preserves_authoritative_decision_fields():
    ...
    result=ReasonerService().analyze(decision,[ev("ev-1","Trend is BULLISH"),ev("ev-2","Trend is BEARISH"),ev("ev-3","Trend is BEARISH")])
